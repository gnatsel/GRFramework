...
//
//  GRAccelerometerManager.swift
//  ToiletCamera
//
//  Created by Gnatsel Reivilo on 14/06/2016.
//  Copyright © 2016 Gnatsel Reivilo. All rights reserved.
//
...
@objc protocol GRAccelerometerManagerDelegate {
...
@objc(GRAccelerometerManager) public class GRAccelerometerManager :NSObject{
  static let sharedInstance = GRAccelerometerManager()
  
  let manager = CMMotionManager()
  var delegate: GRAccelerometerManagerDelegate?
  
  /*
   *  init:void:
   *
   *  Discussion:
   *   Initialises the GRAccelerometerManager class and throw a Log with a timestamp.
   */
  public override init(){
    print("GRAccelerometerManager has been initialised successfully")
  }
  
  public func getAngle(acceleration:CMAcceleration) -> Double {
      return getAngle((x: acceleration.x, y: acceleration.y, z: acceleration.z))
  }
  public func getAngle(acceleration:(x:Double,y:Double,z:Double)) -> Double {
    var angle:Double = 0
    switch (UIDevice.currentDevice().orientation) {
    case .LandscapeLeft:
      angle = atan2(acceleration.x, acceleration.z);
      break;
    case .LandscapeRight:
      angle = atan2(-acceleration.x, acceleration.z);
      break;
    case .Portrait:
      angle = atan2(acceleration.y, acceleration.z);
      break;
  ...
  public func getMagneticFieldAtCurrentInstant (values: (x:Double, y:Double, z:Double) -> ()){
